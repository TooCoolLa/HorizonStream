...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, cams=None, cam_drop=False, attn_mask=None, kv_cache=None, q_cache=None, block_attn_mask=None) -> Tensor:
        def attn_residual_func(x: Tensor, pos=None, scale_msa=None, shift_msa=None, attn_mask=None, kv_cache=None, q_cache=None, block_attn_mask=None) -> Tensor:
            if not self.use_cam_emb or cams is None:
                x = self.attn(self.norm1(x), pos=pos, attn_mask=attn_mask, kv_cache=kv_cache, q_cache=q_cache, block_attn_mask=block_attn_mask)
                return self.ls1(x)
            else:
                ...
                x = self.attn(func(self.norm1(x), shift_msa, scale_msa), pos=pos, attn_mask=attn_mask, kv_cache=kv_cache, q_cache=q_cache, block_attn_mask=block_attn_mask)
                return self.ls1(x)
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            assert attn_mask is None and kv_cache is None and block_attn_mask is None, "attn_mask, kv_cache, block_attn_mask are not supported for drop_add_residual_stochastic_depth yet"
