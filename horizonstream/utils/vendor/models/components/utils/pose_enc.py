...
def pose_encoding_to_extri_intri(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        if build_intrinsics:
            H, W = image_size_hw
            f_default = max(float(H), float(W)) / 2.0
            default_fov_h = 2 * torch.atan(
                torch.tensor(float(H) / (2.0 * f_default), device=pose_encoding.device)
            )
            default_fov_w = 2 * torch.atan(
                torch.tensor(float(W) / (2.0 * f_default), device=pose_encoding.device)
            )
            invalid_h = (~torch.isfinite(fov_h)) | (torch.abs(fov_h) < 1e-6)
            invalid_w = (~torch.isfinite(fov_w)) | (torch.abs(fov_w) < 1e-6)
            fov_h = torch.where(invalid_h, default_fov_h, fov_h)
            fov_w = torch.where(invalid_w, default_fov_w, fov_w)
